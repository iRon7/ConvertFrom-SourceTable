...
begin {
    ...
    $HeaderLine = if ($Header -is [Array] -and $Header.Count -gt 1) { '' } elseif ($Header) { $Header }
    $TopLine = if ($HeaderLine) { '' }
    $LastLine,$OuterLeftColumn,$OuterRightColumn,$Mask = $Null
    $RowIndex = 0; $Padding = 0; $Columns = [System.Collections.Generic.List[HashTable]]::new()
    $Property = [System.Collections.Specialized.OrderedDictionary]::new()             # Include support from PSv2
    ...
    function Mask ([string]$Line,[byte]$Or = [Mask]::Data) {
        ...
        if ($Init) { ([ref]$Mask).Value = [System.Collections.Generic.List[Byte]]::new() }
        ...
        if ($Null -ne $DataIndex) {
            ...
            if ($VRx) {
                ...
                foreach ($Column in $Columns) {
                    if (-not $Column.Type -and -not $Column.Name) { continue }
                ...
                if (!$Ruler -and $HRx) {                    # Connect (ruler-less) single spaced headers where either column is empty
                ...
                for ($i = 0; $i -le $Mask.Count; $i++) {
                    ...
                    elseif ($InWord) {
                        ...
                        if (!$Masked) {
                            ...
                            elseif ($Index -lt @($Header).Count) { TypeName $Header[$Index] }
                            if ($HeaderLine -and -not $Name) { continue }
                            if ($Name -and $Columns.Where{ $_.Name -eq $Name }) { Write-Warning "Duplicate column name: $Name." }
                            if ($Type) {
                            ...
                            $Left = $Column
                            if ($Name) { $Property.Add($Name, $Null) }
                            $Index++
                        }
                    }
                }
                            ...
                            if ($ParseRightAligned -and $Value -gt '') {
                                ...
                                if ($Alignment -eq [Alignment]::Right) {
                                    ...
                                    try { & ([scriptblock]::Create("[$($Column.Type)]`$Value")) }
                                        ...
                                        Write-Error -ErrorRecord (ErrorRecord -Line $Line -Start $Column.Start -End $Column.End -Message (
                                                "The value '{0}' in row {1} at column '{2}' can't be converted to type {1}." -f $Value,$RowIndex,$Column.Name,$Column.Type
                                            ))
                                    }
                                } else { $Value }
                            } else { $Value }
